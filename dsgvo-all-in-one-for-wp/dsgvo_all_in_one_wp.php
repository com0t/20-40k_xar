...
class dsdvo_wp_backend {
	...
	public static function dsgvo_aio_admin_menu(){
		...
		$notification_count_changelog = get_option('dsgvoaio_notification_count_v41', '1');
		...
		public static function dsdvo_wp_add_scripts() {
			...
			if ($is_elementor_preview == false) {
				...
				if ($language == "en") {
					...
					$twitter_layer = html_entity_decode(get_option("dsdvo_twitter_layer_en"), ENT_COMPAT, get_option('blog_charset'));
				...
				if ($language == "it") {
					...
					$twitter_layer = html_entity_decode(get_option("dsdvo_twitter_layer_it"), ENT_COMPAT, get_option('blog_charset'));
